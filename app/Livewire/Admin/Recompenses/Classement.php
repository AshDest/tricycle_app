...
/**
 * Classement des performances des motards
 */
...
class Classement extends Component
{
    ...
    /**
     * Calculer les performances pour le mois sélectionné
     */
    public function calculerPerformances()
    {
        $service = new PerformanceService();
        $service->calculerPerformancesMensuelles($this->mois, $this->annee);

        session()->flash('success', "Performances calculées pour " . $this->getNomMois() . " {$this->annee}");
    }

    /**
     * Attribuer les récompenses automatiquement
     */
    public function attribuerRecompenses()
    {
        $service = new PerformanceService();
        $recompenses = $service->attribuerRecompensesMensuelles($this->mois, $this->annee);

        session()->flash('success', count($recompenses) . " récompenses attribuées pour " . $this->getNomMois() . " {$this->annee}");
    }
    ...
    public function render()
    {
        ...
        return view('livewire.admin.recompenses.classement', [
